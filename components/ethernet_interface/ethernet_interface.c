...
//#define CONFIG_ETH_IF_USE_INTERNAL_ETHERNET 1
//#define CONFIG_ETH_IF_ETH_PHY_LAN87XX 1

#if CONFIG_ETH_IF_USE_SPI_ETHERNET
#define INIT_SPI_ETH_MODULE_CONFIG(eth_module_config, num)                                      \
    do {                                                                                        \
        eth_module_config[num].spi_cs_gpio = CONFIG_ETH_IF_ETH_SPI_CS ##num## _GPIO;           \
        eth_module_config[num].int_gpio = CONFIG_ETH_IF_ETH_SPI_INT ##num## _GPIO;             \
        eth_module_config[num].phy_reset_gpio = CONFIG_ETH_IF_ETH_SPI_PHY_RST ##num## _GPIO;   \
        eth_module_config[num].phy_addr = CONFIG_ETH_IF_ETH_SPI_PHY_ADDR ##num;                \
    } while(0)
...
void ethernet_interface_init(void)
{
...
#if CONFIG_ETH_IF_USE_INTERNAL_ETHERNET
    ...
    phy_config.phy_addr = CONFIG_ETH_IF_ETH_PHY_ADDR;
    phy_config.reset_gpio_num = CONFIG_ETH_IF_ETH_PHY_RST_GPIO;
    gpio_pad_select_gpio(PIN_PHY_POWER);
    gpio_set_direction(PIN_PHY_POWER,GPIO_MODE_OUTPUT);
    gpio_set_level(PIN_PHY_POWER, 1);
    vTaskDelay(pdMS_TO_TICKS(10));
    mac_config.smi_mdc_gpio_num = CONFIG_ETH_IF_ETH_MDC_GPIO;
    mac_config.smi_mdio_gpio_num = CONFIG_ETH_IF_ETH_MDIO_GPIO;
    esp_eth_mac_t *mac = esp_eth_mac_new_esp32(&mac_config);
#if CONFIG_ETH_IF_ETH_PHY_IP101
    esp_eth_phy_t *phy = esp_eth_phy_new_ip101(&phy_config);
#elif CONFIG_ETH_IF_ETH_PHY_RTL8201
    esp_eth_phy_t *phy = esp_eth_phy_new_rtl8201(&phy_config);
#elif CONFIG_ETH_IF_ETH_PHY_LAN87XX
    esp_eth_phy_t *phy = esp_eth_phy_new_lan87xx(&phy_config);
#elif CONFIG_ETH_IF_ETH_PHY_DP83848
    esp_eth_phy_t *phy = esp_eth_phy_new_dp83848(&phy_config);
#elif CONFIG_ETH_IF_ETH_PHY_KSZ8041
    esp_eth_phy_t *phy = esp_eth_phy_new_ksz8041(&phy_config);
#elif CONFIG_ETH_IF_ETH_PHY_KSZ8081
...
#endif //CONFIG_ETH_IF_USE_INTERNAL_ETHERNET
    ...
    esp_netif_t *eth_netif_spi[CONFIG_ETH_IF_SPI_ETHERNETS_NUM] = { NULL };
    char if_key_str[10];
    char if_desc_str[10];
    char num_str[3];
    for (int i = 0; i < CONFIG_ETH_IF_SPI_ETHERNETS_NUM; i++) {
        itoa(i, num_str, 10);
    ...
    // Init SPI bus
    spi_device_handle_t spi_handle[CONFIG_ETH_IF_SPI_ETHERNETS_NUM] = { NULL };
    spi_bus_config_t buscfg = {
        .miso_io_num = CONFIG_ETH_IF_ETH_SPI_MISO_GPIO,
        .mosi_io_num = CONFIG_ETH_IF_ETH_SPI_MOSI_GPIO,
        .sclk_io_num = CONFIG_ETH_IF_ETH_SPI_SCLK_GPIO,
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
    };
    ESP_ERROR_CHECK(spi_bus_initialize(CONFIG_ETH_IF_ETH_SPI_HOST, &buscfg, SPI_DMA_CH_AUTO));

    // Init specific SPI Ethernet module configuration from Kconfig (CS GPIO, Interrupt GPIO, etc.)
    spi_eth_module_config_t spi_eth_module_config[CONFIG_ETH_IF_SPI_ETHERNETS_NUM];
    INIT_SPI_ETH_MODULE_CONFIG(spi_eth_module_config, 0);
#if CONFIG_ETH_IF_SPI_ETHERNETS_NUM > 1
    INIT_SPI_ETH_MODULE_CONFIG(spi_eth_module_config, 1);
#endif

    // Configure SPI interface and Ethernet driver for specific SPI module
    esp_eth_mac_t *mac_spi[CONFIG_ETH_IF_SPI_ETHERNETS_NUM];
    esp_eth_phy_t *phy_spi[CONFIG_ETH_IF_SPI_ETHERNETS_NUM];
    esp_eth_handle_t eth_handle_spi[CONFIG_ETH_IF_SPI_ETHERNETS_NUM] = { NULL };
#if CONFIG_ETH_IF_USE_KSZ8851SNL
    spi_device_interface_config_t devcfg = {
        .mode = 0,
        .clock_speed_hz = CONFIG_ETH_IF_ETH_SPI_CLOCK_MHZ * 1000 * 1000,
        .queue_size = 20
    };
    ...
    for (int i = 0; i < CONFIG_ETH_IF_SPI_ETHERNETS_NUM; i++) {
        ...
        ESP_ERROR_CHECK(spi_bus_add_device(CONFIG_ETH_IF_ETH_SPI_HOST, &devcfg, &spi_handle[i]));
    ...
    if (bits & NETWORK_CONNECTED_BIT) {
      ...
      ESP_LOGI(TAG, "Failed to connect via ethernet");
    } else {
